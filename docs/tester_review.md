...
## Next step (already specified to run)

Combine only filters that survived independently, on the same window/risk:

| Run | Min_HTF_Source_Body_Percent | Max_Entry_Distance_R | Min_Zone_Age_Minutes |
|---|---|---:|---:|---:|
| A | 20 | 0 | 90 |
| B | 0 | 0.25 | 90 |
| C (all) | 20 | 0.25 | 90 |
...
Then re-run the best combination at RR 1, 2 and 3 (three more passes). After the combination + RR passes, freeze the parameters and validate on genuinely unseen data (live/forward window after 2026-09-08, or at minimum a 2021+ holdout reported separately) before any claim of improvement. In parallel, implement deal-level profit logging (realized profit, commission, swap per ticket) so the analysis no longer depends on the 100 oz/lot gross reconstruction.