...
Higher-timeframe areas of interest, matching lower-timeframe entries, fixed lots, wick-based SL and reward/risk TP. This version can submit market orders when attached with trading enabled. It has been compiled but has not been attached to a live chart or run against a broker during development.
...
| Lot_Size | 0.01 | Fixed volume; invalid broker min/max/step values are rejected rather than rounded up |
...
Refresh Navigator, select DhanuFX\DhanuFX, and start a fresh visual test with M90/M5, 0.01 lots and RR 2.0. Use Every tick based on real ticks for the most useful execution test. Every tick or 1 minute OHLC can also exercise the logic, but modeled intrabar paths can change retests, invalidation and SL/TP results. Avoid coarse Open prices only runs for multi-timeframe execution.
...
Test-TradeRules.ps1 executes shared MQL trade-decision/calculation bodies through .NET with syntax adaptations. All 31 checks passed: direction matching, retests, pre-activation rejection, expiry, consumed zones, wick-stop invalidation, bid/ask RR calculations, tick-size rounding and invalid stop/quote rejection. These checks do not submit orders or replace an MT5 runtime/backtest.
